...
public class MainController {
    ...
    @GetMapping("/groot")
    public ResponseEntity<?> groot (@RequestParam(required = false) String text) {
        if (text == null) {
    ...
    @GetMapping("/arrow")
    public ResponseEntity<?> arrow (@RequestParam(required = false) Integer distance, @RequestParam(required = false) Integer time) {

        if (distance == null && time == null ) {
            return new ResponseEntity<>(new Error("Missing distance and time parameter"), HttpStatus.BAD_REQUEST);
        }

        else if (time == null) {
            return new ResponseEntity<>(new Error("Missing time parameter"), HttpStatus.BAD_REQUEST);
        }

        else if (distance == null) {
            return new ResponseEntity<>(new Error("Missing distance parameter"), HttpStatus.BAD_REQUEST);
        }

        else if (time == 0 || distance == 0) {
            return new ResponseEntity<>(new Error("Parameters cannot be 0"), HttpStatus.BAD_REQUEST);
        }
